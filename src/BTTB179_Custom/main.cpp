...
constexpr const char *DEVICE_NAME = "BTTB179 Custom";

BLECentralProfileBTTB179 BTTB179;
...
bool is_rotate_encoder0_called = false;
bool is_rotate_encoder1_called = false;
Set ids;
...
void trackball_report_callback(uint8_t buttons, int16_t x, int16_t y, int8_t wheel, int8_t pan)
{
  ...
  {
    wheel_sum += wheel;
    if (is_rotate_encoder0_called == false)
    {
      HidEngine.rotateEncoder(EncWheel);
      is_rotate_encoder0_called = true;
    }
  }
  ...
  {
    pan_sum += pan;
    if (is_rotate_encoder1_called == false)
    {
      HidEngine.rotateEncoder(EncPan);
      is_rotate_encoder1_called = true;
    }
  }
...
void read_encoder_step_callback(EncoderId encoder_id, int16_t &step)
{
  ...
  if (encoder_id == EncWheel)
  {
    step = wheel_sum;
    wheel_sum = 0;
    is_rotate_encoder0_called = false;
  }
  else if (encoder_id == EncPan)
  {
    step = pan_sum;
    pan_sum = 0;
    is_rotate_encoder1_called = false;
  }
  xSemaphoreGive(callback_mutex);
}
...
void setup()
{
  ...
  // BTTB179 Host
  BTTB179.begin();
  BTTB179.Hid.setTrackballReportCallback(trackball_report_callback);

  // ConnectionController
  ScanLed.begin();
  Bluefruit_ConnectionController.begin();
  Bluefruit_ConnectionController.Central.setProfile(&BTTB179);
